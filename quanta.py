...
def predict(model, original_data, preprocessor, input_data, categorical_cols):
    ...
    if prediction[0] == 1:
        st.success('O participante fará portabilidade.')
    else:
        st.error('O participante não fará portabilidade.')

    st.write('### Probabilidades:')
    st.write(f'Probabilidade de não fazer portabilidade: {prediction_proba[0][0]*100:.2f}%')
    st.write(f'Probabilidade de fazer portabilidade: {prediction_proba[0][1]*100:.2f}%')
...
if uploaded_file is not None:
    ...
    # Após a previsão, deixar a parte de baixo em branco
    st.empty()
